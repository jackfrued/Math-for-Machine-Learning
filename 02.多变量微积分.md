...
$$
\mathbf{J}_{\mathbf{g}}(\mathbf{x}) = \begin{bmatrix} \frac{\partial g_1}{\partial x_1} & \cdots & \frac{\partial g_1}{\partial x_n} \\\\ \vdots & \ddots & \vdots \\\\ \frac{\partial g_m}{\partial x_1} & \cdots & \frac{\partial g_m}{\partial x_n} \end{bmatrix}
$$

你可以把它理解为：在 $\small{\mathbf{x}}$ 附近，用一个线性变换近似 $\small{\mathbf{g}}$ ：

$$
g({x}+\Delta{x}) \approx {g}({x}) + {J}_{g}({x}) \Delta{x}
$$
...
$$
\ell(\mathbf{w},b)=\frac{1}{2}\left(\mathbf{w}^T\mathbf{x}+b-y\right)^2
$$

令 $\small{e=\mathbf{w}^T\mathbf{x}+b-y}$ ，则：

$$
\nabla_{\mathbf{w}}\ell = e\,\mathbf{x}
$$

$$
\frac{\partial \ell}{\partial b} = e
$$
