...
1. **多变量函数**（*multivariable function*）：输入是多个变量，输出是一个数或一个向量的函数。  
   - **标量值函数**：$f:\mathbb{R}^n\rightarrow\mathbb{R}$，例如损失函数 $L(\theta)$。  
   - **向量值函数**：$\mathbf{g}:\mathbb{R}^n\rightarrow\mathbb{R}^m$，例如神经网络的中间层输出 $\mathbf{h}(\theta)$。

2. **点**与**邻域**：在 $\mathbb{R}^n$ 中，一个参数向量 $\mathbf{x}=[x_1,\dots,x_n]^T$ 就是一个点。所谓“在某点附近”，就是在它的**邻域**里做微小扰动。
...
**偏导数**（*partial derivative*）的意思是：当我们研究 $f(x,y)$ 对 $x$ 的变化时，先把 $y$ 当成常数。

对二元函数 $f(x,y)$，**偏导数**定义为：

$$
\frac{\partial f}{\partial x}(x,y)
= \lim_{h\to 0}\frac{f(x+h,y)-f(x,y)}{h} \tag{1}
$$
...
= \lim_{h\to 0}\frac{f(x,y+h)-f(x,y)}{h} \tag{2}
$$

如果 $f$ 有 $n$ 个变量，就有 $n$ 个偏导数 $\frac{\partial f}{\partial x_i}$。

**例子**：令 $f(x,y)=x^2y+3y$，则  
- 对 $x$ 求偏导（把 $y$ 当常数）：
...
当 $f:\mathbb{R}^n\rightarrow\mathbb{R}$ 时，我们把所有偏导数组成一个向量，得到 **梯度**（*gradient*）：
...
其中 **学习率**（*learning rate*）$\eta>0$ 控制每一步走多远。

### 方向导数：沿着任意方向看“变化率”

有时我们不想只看坐标轴方向（偏导数），而是想沿任意方向 $\mathbf{u}$（通常取单位向量）看变化率，这就是 **方向导数**（*directional derivative*）。
...
=
\lim_{h\to 0}\frac{f(\mathbf{x}+h\mathbf{u})-f(\mathbf{x})}{h} \tag{7}
$$

如果 $f$ 在 $\mathbf{x}$ 可微，则有一个非常漂亮的结论：
...
+
\frac{\partial f}{\partial y}\frac{dy}{dt} \tag{11}
$$
...
当输出是向量时：$\mathbf{g}:\mathbb{R}^n\rightarrow\mathbb{R}^m$，我们用 **雅可比矩阵**（*Jacobian matrix*）记录所有一阶偏导：
...
你可以把它理解为：在 $\mathbf{x}$ 附近，用一个线性变换近似 $\mathbf{g}$：
...
\mathbf{g}(\mathbf{x})
+
\mathbf{J}_{\mathbf{g}}(\mathbf{x})\,\Delta\mathbf{x} \tag{13}
$$
...
当 $f:\mathbb{R}^n\rightarrow\mathbb{R}$ 时，二阶偏导组成的矩阵叫 **海森矩阵**（*Hessian matrix*）：
...
直觉上：  
- **梯度**告诉你“往哪走”；  
- **海森矩阵**告诉你“地形陡不陡、弯不弯”，也就是**曲率**。

这也是为什么二阶方法（如牛顿法）更“聪明”，因为它不仅看方向，还看曲率。

### 二阶泰勒展开：把函数在局部“拍扁”

在一元情况下，我们用泰勒展开做近似；多变量也一样。对可二次可微的 $f$，在 $\mathbf{x}$ 附近：
...
+
\frac{1}{2}\Delta\mathbf{x}^T\mathbf{H}_f(\mathbf{x})\Delta\mathbf{x} \tag{15}
$$
