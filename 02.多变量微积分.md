...
$$
\frac{\partial f(x,y)}{\partial x} = \lim_{h\to 0}\frac{f(x+h,y)-f(x,y)}{h} \tag{1}
$$

同理：

$$
\frac{\partial f(x,y)}{\partial y} = \lim_{h\to 0}\frac{f(x,y+h)-f(x,y)}{h} \tag{2}
$$
...
$$
\nabla f(x) = \begin{bmatrix} \frac{\partial f}{\partial x_1} \\\\ \frac{\partial f}{\partial x_2} \\\\ \vdots \\\\ \frac{\partial f}{\partial x_n} \end{bmatrix} \tag{5}
$$
...
$$
D_{u}f(x) = \lim_{h\to 0} \frac{f(x + hu)-f(x)}{h} \tag{7}
$$
...
$$
g({x}+\Delta{x}) \approx {g}({x}) + {J}_{g}({x}) \Delta{x} \tag{13}
$$
